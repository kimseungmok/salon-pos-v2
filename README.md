...
- **구현**: M1(상품/카테고리), M2(직원초대+시프트데이터), M3(고객+그룹자동분류), M4(예약등록로직+취소/노쇼 예약금처리+웨이팅), M5(주문+결제+분할결제로직+취소원자처리) 완료. 다음은 M6(선불권, 신규) — `design/spec/v3/IMPLEMENTATION_PLAN.md` §4 표 순서대로 진행
