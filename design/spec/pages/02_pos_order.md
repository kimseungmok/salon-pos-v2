...
> **2026-06-22 비고**: 이 화면의 「✏ グリッド編集」모드(표시/숨기기·삭제·빈칸에 商品追加·페이지 추가/삭제)가 토스포스 §3-1 "즐겨찾는 메뉴·옵션 편집모드"를 그대로 구현한 것이며, **대기 중이던 `27_product_layout`(상품 배치 편집) 요구사항을 이미 충족**함. 별도 화면을 새로 만들지 않고 여기로 흡수 — `INDEX.md` 참조.
...
> ⚠️ `＋ 商品追加` 버튼 제거됨. 상품 추가는 편집 모드 진입 후 빈 슬롯 클릭(→ 피커 모달 J)으로만 가능.
...
*최종 수정: 2026-06-16 (＋商品追加 버튼 제거, 피커 모달로 통일) | 기준 모크업: `design/mockups/v2/ja/02_pos_order.html`*
