...
│ ┌──────────────────────────────────────────┐ │ ┌───────────────────────────┐ │
│ │ B. 좌측 상단바 52px                       │ │ │ K. 헤더 ~42px             │ │
│ │  ← 注文に戻る  会計  [¥17,160 を決済する→]│ │ │  🖊 注文内容  (네이비 배경) │ │
│ └──────────────────────────────────────────┘ │ └───────────────────────────┘ │
│ ┌──────────────────────────────────────────┐ │ ┌───────────────────────────┐ │
│ │ C. 금액 존                                │ │ │ K. 아이템 목록 (스크롤)   │ │
│ │  お支払金額              [⚡ 分割払い]    │ │ │                           │ │
│ │  ¥17,160                                 │ │ │  カット 10%割引    ¥3,960 │ │
│ │  消費税込み ¥1,560                        │ │ │   원가 ¥4,400 취소선       │ │
│ └──────────────────────────────────────────┘ │ │                           │ │
│ ┌──────────────────────────────────────────┐ │ │  ワンカラー        ¥8,800 │ │
│ │ D. 칩 행                                  │ │ │                           │ │
│ │  [👤 田中美咲·320pt] [🎟クーポン適用] ⭐  │ │ │  トリートメント M  ¥4,400 │ │
│ └──────────────────────────────────────────┘ │ │                           │ │
│ ┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄ │ │  (↕ overflow-y: auto)     │ │
│ ┌──────────────────────────────────────────┐ │ └───────────────────────────┘ │
│ │ E. 결제수단 그리드                         │ │ ┌───────────────────────────┐ │
│ │  支払方法                                 │ │ │ K. 합계 푸터              │ │
│ │  [💴現金][💳カード][📱PayPay]             │ │ │  小計          ¥17,600    │ │
│ │  [💬LINE Pay]        [📋後払い]           │ │ │  割引合計       -¥440     │ │
│ └──────────────────────────────────────────┘ │ │  ─────────────────────    │ │
│ ┌──────────────────────────────────────────┐ │ │  お支払金額    ¥17,160    │ │
│ │ F. 하단 요약 (margin-top:auto)            │ │ │  消費税込み ¥1,560        │ │
│ │  割引: -¥440  ポイント: -  メモ: …        │ │ │  ⭐ +171pt 獲得予定        │ │
│ └──────────────────────────────────────────┘ │ └───────────────────────────┘ │
└──────────────────────────────────────────────┴───────────────────────────────┘
```
...
```
┌─────────────────────────────┐  ← .pay-right  width:320px
│          K. 헤더             │  ← .right-header  background:#1C2833
│  🖊 注文内容                 │    font:12px/600/white
│                             │    height:~42px
├─────────────────────────────┤
│          K. 아이템 목록      │  ← .right-list  flex:1  overflow-y:auto
│                             │
│  ┌─────────────────────┐    │  ← .oi (반복)  border-bottom:.5px solid #F5F5F3
│  │ カット   10%割引      │    │    .oi-name  12px/500
│  │          ¥4,400̶ ¥3,960│    │    .oi-orig  10px/취소선/#CCC  (할인 있을 때만)
│  │ ×1                   │    │    .oi-price  12px/600
│  └─────────────────────┘    │    .oi-price.disc  color:#E74C3C
│  ┌─────────────────────┐    │
│  │ ワンカラー            │    │
│  │              ¥8,800 │    │
│  │ ×1                   │    │
...
│                             │
├─────────────────────────────┤
│          K. 합계 푸터        │  ← .right-footer  flex-shrink:0
│  小計            ¥17,600    │    .rf-row  font:12px
│  割引合計          -¥440    │    .rf-disc  color:#E74C3C
│  ポイント使用       -¥0     │    (ptsUsed>0 일 때만 표시)
│  ─────────────────────────  │    .rf-divider
│  お支払金額      ¥17,160    │    .rf-total-val  24px/700
│  消費税込み ¥1,560           │    .rf-tax  10px/#BBB
│  ⭐ +171pt 獲得予定          │    .rf-earn  11px/#2E86C1
└─────────────────────────────┘
```
