...
### 결정 완료 — 예약 취소/노쇼 시 예약금 처리 (마지막 미해결 이슈, 2026-06-23 3차 검토)

**결정**: F-PAY-05(결제취소 원자적 처리)와 동일 패턴을 적용하는 **F-BOOK-04**를 신설.

| 상황 | 처리 |
|---|---|
| 24시간 전 취소 | 전액 환불 |
| 24시간 이내 취소 / 노쇼 | 환불 불가, 매장 매출로 귀속 |
| 매장 사정 취소 | 전액 환불 |

- `booking/data_spec.md`에 `cancelBooking()` 추가, 환불 처리는 `payment_pos/data_spec.md`에 신설한 범용 `refundPayment()`를 재사용(중복 구현 방지)
- `Payment` 엔티티에 `status`(`completed`/`refunded`) 필드가 누락되어 있던 것도 이번에 함께 보강

---

## 🎉 전체 결론

1차(2026-06-23) + 2차 + 3차 검토를 거쳐 **모든 이슈가 해소됨**. v3 정의서 10개 영역(27+3=30개 문서)은 영역 간 용어·엔티티·함수 참조가 전부 일치하는 상태로 확정.
